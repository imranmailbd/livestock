...
class DatabaseUpdate{
	...
	*/
			...
			while($oneRow = $tableObj->fetch(PDO::FETCH_OBJ)){
				...
				if($missingOnPOS){
					...
					while($oneRow2 = $missingOnPOS->fetch(PDO::FETCH_OBJ)){
						...
						if($oneRow2->item_type !='livestocks'){
	...
	public function updateRepairShippingQty(){
		$repairsObj = $this->db->query("SELECT pos.accounts_id, pos.pos_id, pos_cart.pos_cart_id, pos_cart.item_id, pos_cart.shipping_qty FROM pos, pos_cart, repairs WHERE pos.pos_type = 'Repairs' AND repairs.status NOT IN ('Finished', 'Invoiced', 'Cancelled') AND pos_cart.shipping_qty>0 AND pos.order_status=1 AND pos_cart.item_type !='livestocks' AND pos.pos_id = repairs.pos_id AND pos.pos_id = pos_cart.pos_id ORDER BY pos.pos_id ASC, pos_cart.pos_cart_id ASC LIMIT 0, 10000", array());
		if($repairsObj){
			while($oneRow = $repairsObj->fetch(PDO::FETCH_OBJ)){
				$accounts_id = $oneRow->accounts_id;
				...
				if($dataCount<=10){
					...
					if(!empty($ProductIdsMissingItem)){
						...
						foreach($ProductIdsMissingItem as $product_id=>$productItems){
							$mproduct_id = 0;
							$productObj = $this->db->querypagination("SELECT product_id FROM product WHERE accounts_id = $prod_cat_man AND product_name = '$productName' AND product_type='Live Stocks' ORDER BY product_id DESC LIMIT 0, 1", array());
							if($productObj){
								$mproduct_id = $productObj[0]['product_id'];
							}
							if($mproduct_id==0){
	...
	public function checkArchivedProduct(){
		...
		if($tableObj){	
			...
			while($oneRow = $tableObj->fetch(PDO::FETCH_OBJ)){
				...
				$need = $have = $onPO = 0;
				if(in_array($oneRow->product_type, array('Standard', 'Live Stocks'))){
