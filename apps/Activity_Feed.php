...
class Activity_Feed{
	...
	public function getHMoreInfo($table_id, $tablename, $userIdNames, $activity_feed_title, $sitem_id=0, $fieldname='item_id'){
			...
			if($posRow){
				...
				if(!empty($sitem_id) && $sitem_id !=0){
					if($fieldname=='product_id'){
						...
						$sql2nd = "SELECT pos_cart.* FROM pos_cart, pos_cart_item WHERE pos_cart.pos_id = $pos_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'livestocks' 
				...
				if($query2nd){
					while($onePosRow=$query2nd->fetch(PDO::FETCH_OBJ)){
						...
						if(strcmp($item_type, 'livestocks')==0){
		...
		elseif(strcmp($tablename,'posreturn')==0){
			$subSql = "SELECT pos_cart.description, pos.invoice_no, pos.sales_datetime, pos.user_id FROM pos, pos_cart, pos_cart_item WHERE pos.pos_id = $table_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'livestocks' 
