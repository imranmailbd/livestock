import {
	...
	showTopMessage, setOptions, getDeviceOperatingSystem, popup_dialog, popup_dialog600, validateRequiredField, fetchData, 
...
export async function categoryPPProduct(){
	...
	function getClickHandler(info,frompage){
		...
		if(['POS', 'Inventory_Transfer'].includes(frompage) && info.product_type==='Live Stocks'){
...
export function showOrNotSquareup(){
	let operSys = getDeviceOperatingSystem();
...
export function checkMethod(){
	...
	if(document.querySelector("#method") && document.querySelector("#method").value==='Squareup'){	 
		const operSys = getDeviceOperatingSystem();
