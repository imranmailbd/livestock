...
'cell_phones':Translate('Live Stocks'), 'Quote':Translate('Request a Quote'), 'Appointment': Translate('Repair Appointment'), 'RStatus': Translate('Check Repair Status'),'preview': Translate('Preview Website')};
						...
						let customizeRow = cTag('div',{ 'class':'flexSpaBetRow' });					
							...
							customizeColumn.appendChild(cTag('hr'));
								...
								customizeDiv.appendChild(cTag('br'));
									...
									inventoryLabel.append(' '+Translate('Display Live Stocks Link')+' ');
...
async function AJ_lists_MoreInfo(){
	...
	function afterFetch(data){
		...
		[
			...
			{id:'cellPhonesWidgetCode',class:'cellPhones', title:Translate('Widget for checking Live Stocks'),info:Translate('Just copy the code below and paste it into your HTML where you want your customers to see the Live Stocks.')},
...
function headerPreview(){
	...
	if(display_inventory>0){
		linkCreator(PagesHeader,'Livestock',Translate('Live Stocks'));
	}
...
async function cell_phones(){
    const showTableData = document.getElementById("viewPageInfo");
        header(Translate('Live Stocks'));
		const cellPhoneRow = cTag('div',{ 'class':"flexSpaBetRow" });
            let cellPhoneColumn = cTag('div',{ 'class':"columnXS12" });
                let callOutDiv = cTag('div',{ 'class':"innerContainer",'style':"margin-top: 0; background: #fff;" });
                    const form = cTag('form',{ 'name':"frmLivestock",'id':"frmLivestock",'action':"#",'enctype':"multipart/form-data",'method':"post",'accept-charset':"utf-8" });
                        let cellPhoneDiv = cTag('div',{ 'class':"flexStartRow" });
						[
							{label:Translate('Display Live Stocks Link'),id:'display_inventory'},
...
async function AJ_cell_phones_MoreInfo(){
	...
	document.querySelector('#frmLivestock').addEventListener('submit',AJsave_cell_phones);
}
