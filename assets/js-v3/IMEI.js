...
const listsFieldAttributes = [
    {align:'left', 'data-title':Translate('Model')},
    {align:'center', 'data-title':Translate('Color')},
    {align:'center', 'data-title':Translate('GB')},
    {align:'center','data-title':Translate('Cond')},
    {align:'center','data-title':Translate('Carrier')},
    {align:'right', 'data-title':Translate('IMEI Number')},
    {align:'right', 'data-title':Translate('PO')+' #'},
    {align:'center', 'class':'hidefor991-768', 'data-title':Translate('Date Entered')},
    {align:'center', 'class':'hidefor991-768', 'data-title':Translate('Invoice')+' #'}
];
...
async function filter_IMEI_lists(){
    ...
    function afterFetch(data){
        ...
        option.innerHTML = Translate('All Device Models');
        ...
        tableData.forEach(item=>{
            ...
            item.forEach((itemInfo,index)=>{
                ...
                if(index===7){
                    ...
                    const aTag = cTag('a',{'class':'anchorfulllink','href':`/${uriStr}/${item[0]}`});
...
function lists(){
    ...
    [
        ...
        const titleRow = cTag('div', {'class':'outerListsTable','style': "padding: 5px; text-align: start;"});
            ...
            headerTitle.innerHTML = Translate('Devices Inventory');
    ...
    showTableData.appendChild(titleRow);
        ...
        filterRow = cTag('div', {class: "flexEndRow outerListsTable"});
            sortDropDown = cTag('div', {class: "columnXS6 columnLG2"});
                ...
                [
                    { value: 1, label: Translate('Devices in Inventory')},
                    { value: 0, label: Translate('Devices not in Inventory')},
                    { value: 2, label: Translate('All Devices')},
                ].forEach(field=>{
            ...
            filterRow2.appendChild(sortDropDown);
                ...
                const searchDiv = cTag('div', {class: "columnXS6 columnSM4"});
                    const SearchInGroup = cTag('div', {class: "input-group"});
                        const searchField = cTag('input', {'keydown':listenToEnterKey(filter_IMEI_lists),'type': "text", 'placeholder': Translate('Search IMEI'), 'value': "", id: "keyword_search", name: "keyword_search", class: "form-control", 'maxlength':50,});
                    SearchInGroup.appendChild(searchField);
                        const searchSpan = cTag('span', {class: "input-group-addon cursor", 'click': filter_IMEI_lists, 'data-toggle':"tooltip", title: "", 'data-original-title': Translate('Search IMEI')});
                ...
                const listTable = cTag('table', {class: "table-bordered table-striped table-condensed cf listing"});
                    const listHead = cTag('thead', {class: "cf"});
                        ...
                        const listHeadRow = cTag('tr',{class:'outerListsTable'});
                            ...
                            const thCol2 = cTag('th', {'width': "5%"});
                            thCol2.innerHTML = columnNames[2];

                            const thCol3 = cTag('th', {'width': "6%"});
...
function view(){
    ...
    showTableData.innerHTML = '';
        const titleRow = cTag('div', {class: "flexSpaBetRow", 'style': "padding: 5px;"});
            const headerTitle = cTag('h2');
            headerTitle.innerHTML = Translate('Device Information')+' ';
        ...
        titleRow.appendChild(headerTitle);
            const inventoryListLink = cTag('a', {class: "btn defaultButton", 'href':"/IMEI/lists", title:Translate('Devices Inventory')});
            inventoryListLink.append(cTag('i', {class: "fa fa-list"}), ' ', Translate('Devices Inventory'));
    ...
    showTableData.appendChild(viewHeaderColumn);
        ...
        const viewContentRow = cTag('div', {class: "flex"});
            ...
            viewContentColumn.appendChild(historyTable(Translate('IMEI History'),hiddenProperties));
                            ...
                            const basicInfoColumn = cTag('div', {class: "columnXS12 columnSM6", 'align': "left"});
                                const imeiNumberRow = cTag('div', {class: "flex"});
                                    const imeiNumberColumn = cTag('div', {class: "columnSM4"});
                                        ...
                                        imeiNumberLabel.innerHTML = Translate('IMEI Number');
                                ...
                                imeiNumberRow.appendChild(imeiNumberColumn);
                                    ...
                                    const imeiNumberField = cTag('div', {class: "columnSM8"});
                                        let inputField = cTag('input', {'type': 'text', 'required': "required", class: "form-control imeinumber", name: "item_number", id: "item_number", 'value': storedData.item_number, 'maxlength': 20, 'placeholder': Translate('IMEI Number')});
...
async function AJsave_IMEI(hidePopup){			
    ...
    function afterFetch(data){
        if(data.savemsg !=='error' && data.id>0){
            hidePopup();
			window.location = '/IMEI/view/'+data.item_number+'/'+data.savemsg;
		}
		else{
