...
define('swVersion', 'v3');//Need to change name on widget.js file also
define('printsJS', 'prints.js');
define('commonJS', 'common.js');

if(!in_array(OUR_DOMAINNAME, array('livestock.com', 'livestockl.com'))){
	echo 'There has been a error with your request.  If you have questions please email support@skitsbd.com';
	exit;
}

if(in_array(OUR_DOMAINNAME, array('livestockl.com'))){error_reporting(-1);}
else{error_reporting(0);}

$timezone = $_SESSION['timezone']??'Asia/Dhaka';
date_default_timezone_set($timezone);

if(OUR_DOMAINNAME=='livestock.com'){
...
$commonJSmodule = '';
if(in_array($segment2name, array('Account', 'Accounts_Receivables', 'Activity_Feed', 'Admin', 'Appointment_Calendar', 'Commissions', 'Customers', 'Suppliers', 'Dashboard', 'End_of_Day', 'Expenses', 'Getting_Started', 'IMEI', 'Home', 'Integrations', 'Inventory_Transfer', 'Inventory_reports', 'Invoices', 'Manage_Data', 'Orders', 'POS', 'Products', 'Purchase_orders', 'Repairs_reports', 'Sales_reports', 'Settings', 'Repairs', 'Stock_Take', 'Time_Clock', 'Website' ))){
    $commonJSmodule = 'common.module.js';
}
define('commonJSmodule', $commonJSmodule);
...
$filename = "../application/languages/$loadLangFile.php";
...
if (file_exists($filename)) {
	...
	$serverPath = getcwd();
	require_once ("$serverPath/application/languages/$loadLangFile.php");
}
...
if(in_array($segment2name, array('run_daily_cron', 'Createbarcode', 'widget', 'unsubscribe'))){
	include "../application/$segment2name.php";
	exit;
}
...
if(!in_array($subdomain, array('', 'www')) && in_array($segment1name, array('', 'index.php')) && in_array($segment2name, array('', 'Instancehome', 'Contact-Us', 'Customer', 'Services', 'Product', 'CellPhones', 'Quote', 'Appointment', 'Check_Repair_Status'))){
	$segment2 = '';
	if(in_array($segment2name, array('Contact-Us', 'Customer', 'Services', 'Product', 'CellPhones', 'Quote', 'Appointment', 'Check_Repair_Status'))){
		$segment2 = $segment2name;
	}	
	$clsObj = new Instancehome($db);
	$clsFuncNames = get_class_methods($clsObj);
	$viewFunctions = array('index'=>stripslashes(COMPANYNAME.' Software'), 'Contact-Us'=>$db->translate('Contact Us'), 'Customer'=>$db->translate('Add Customer Information'), 'Services'=>$db->translate('Services'), 'Product'=>$db->translate('Products'), 'CellPhones'=>$db->translate('Mobile Devices'), 'Quote'=>$db->translate('Request a Quote'), 'Appointment'=>$db->translate('Repair Appointment'), 'Check_Repair_Status'=>$db->translate('Check Repair Status Online'));
...
if(!in_array($segment2name, array('Home', 'POS', 'Repairs', 'Invoices', 'Customers', 'Suppliers', 'Products', 
