...
class Accounts{
	...
	private $account_type, $sub_group_id, $parent_ledger_id, $visible_on, $keyword_search, $ledger_id, $fdate, $date_range, $CountList, $publish, $voucher_type, $subGroOpt, $parLedOpt, $vouTypOpt, $totalRows, $tableRows, $page, $rowHeight;
	...
	public function AJgetSubGroupPopup(){
		...
		$sub_group_id = intval(trim($POST['sub_group_id']??0));
		$account_type = 0;
		$subGroupData = array();
		$subGroupData['login'] = '';
	...
	public function AJsaveSubGroup(){
		...
		if($prod_cat_man==0){
			...
			$savemsg = '';
			$returnStr = 'Ok';
			$sub_group_id = $POST['sub_group_id']??0;
			$account_type = $POST['account_type']??0;
	...
	private function subGroupData(){
		...
		if(array_key_exists($sdata_type, $publishData)){
			$filterSql .= " AND sub_group_publish = $publishData[$sdata_type]";
		}
		if($keyword_search !=''){
		...
		if($CountList == 'Count'){
			...
			if($dataObj){
				...
				foreach($dataObj as $oneRow){
					...
					$tableRows[] = array($sub_group_id, stripslashes($account_type), stripslashes($oneRow['name']), intval($oneRow['sub_group_publish']));
				}				
			}
			return $tableRows;
		}
			...
			if($account_type>0){
				...
				if($subGroOpts){
					while($oneRow = $subGroOpts->fetch(PDO::FETCH_OBJ)){
						...
						$subGroOpt[$oneOptVal] = $oneOptLabel;
					}
				}
			}
			if($callFunc==1){
	...
	public function AJgetPage_ledger(){
		...
		if($prod_cat_man==0){
			...
			$faccount_type = $POST['faccount_type']??0;
			$fsub_group_id = $POST['fsub_group_id']??0;
			$fparent_ledger_id = $POST['fparent_ledger_id']??0;
			...
			$subGroOpt = array(0=>'All Sub-Group');
			$parLedOpt = array(0=>'All Parent Ledger');
			$tableRows = '';
			if(!empty($ledgerData)){
				...
				if($tableObj){
					while($oneRow = $tableObj->fetch(PDO::FETCH_OBJ)){
						if(array_key_exists($oneRow->sub_group_id, $subGroIds)){
							$subGroOpt[$oneRow->sub_group_id] = stripslashes(trim($oneRow->name));
						}
			...
			if(!empty($filterSql)){
				...
				if(!empty($allLedIds)){
					foreach($allLedIds as $sparent_ledger_id=>$parMoreRows){
						...
						if(!empty($ledgerOneRow)){
							...
							$parLedOpt[$sparent_ledger_id] = $sname;
				...
				if(!empty($parLedIds)){
					...
					foreach($parLedIds as $sparent_ledger_id=>$oneParLedInfo){
						...
						$arrow = 0;
						$tableRows[] = array($oneLedInfo1[0], $oneLedInfo1[1], $oneLedInfo1[2], $oneLedInfo1[3],  $debitCredits[$oneLedInfo1[4]], $debitCredits[$oneLedInfo1[5]], $oneLedInfo1[6], $oneLedInfo1[7], $editPer, $hidePer, $arrow, $sparent_ledger_id, $oneLedInfo1[8]);
						...
						if(!empty($parLedInfo)){
							...
							foreach($parSubLedIds as $sparent_sub_ledger_id=>$oneParSubLedInfo){
								...
								$tableRows[] = array($oneLedInfo2[0], $oneLedInfo2[1], $oneLedInfo2[2], $oneLedInfo2[3],  $debitCredits[$oneLedInfo2[4]], $debitCredits[$oneLedInfo2[5]], $oneLedInfo2[6], $oneLedInfo2[7], $editPer, $hidePer, $arrow, $sparent_sub_ledger_id, $oneLedInfo2[8]);
								if($oneLedInfo1[0] != $oneLedInfo2[0]){
									//$tableRows .= "|$sparent_sub_ledger_id|";
								}
								if(!empty($parSubLedInfo)){
									...
									foreach($parSub2LedIds as $sparent_sub2_ledger_id=>$oneParSub2LedInfo){
										...
										$tableRows[] = array($oneLedInfo3[0], $oneLedInfo3[1], $oneLedInfo3[2], $oneLedInfo3[3],  $debitCredits[$oneLedInfo3[4]], $debitCredits[$oneLedInfo3[5]], $oneLedInfo3[6], $oneLedInfo3[7], $editPer, $hidePer, $arrow, $sparent_sub2_ledger_id, $oneLedInfo3[8]);
										if($oneLedInfo1[0] != $oneLedInfo3[0] || $oneLedInfo3[0] != $oneLedInfo2[0]){
											//$tableRows .= "|$sparent_sub2_ledger_id|";
										}
										if(!empty($parSub2LedInfo)){													
											...
											foreach($sledgerIds as $sledger_id=>$oneLedInfo){
												...
												if($oneLedInfo1[0] != $oneLedInfo4[0] || $oneLedInfo2[0] != $oneLedInfo4[0] || $oneLedInfo3[0] != $oneLedInfo4[0]){
													//$tableRows .= "|$sledger_id|";
												}
												$tableRows[] = array($oneLedInfo4[0], $oneLedInfo4[1], $oneLedInfo4[2], $oneLedInfo4[3],  $debitCredits[$oneLedInfo4[4]], $debitCredits[$oneLedInfo4[5]], $oneLedInfo4[6], $oneLedInfo4[7], $editPer, $hidePer, $arrow, $sledger_id, $oneLedInfo4[8]);
											}
										}
									}
								}
				...
				if(!empty($allLedIds)){
					foreach($allLedIds as $parent_ledger_id=>$moreRows){
						if(array_key_exists('0', $moreRows)){
							...
							$parLedOpt[$parent_ledger_id] = $name;
						}
					}			
				}
				...
				if(!empty($parLedIds)){
					...
					foreach($parLedIds as $parent_ledger_id=>$parLedName){
						...
						$tableRows[] = array($accountType, $subGroupName, $name, $visible_on, $debit, $credit, $ledger_publish, $closing_date, $editPer, $hidePer, $arrow, $ledger_id, $ledger_count);
	...
	function AJgetLedgerPopup(){
		...
		if($prod_cat_man==0){
			...
			$sparent_ledger_id = intval(trim($POST['sparent_ledger_id']??0));
			$sparObj = false;
	...
	function setParLedOpt($sub_group_id, $post=0){
		...
		if($sub_group_id>0){
			...
			if($tableObj){
				while($oneRow = $tableObj->fetch(PDO::FETCH_OBJ)){
					$parLedOpt[$oneRow->ledger_id] = stripslashes(trim($oneRow->name));
				}
			}
		}
		...
		if($parent_sub_ledger_id>0){
			...
			if($tableObj){
				foreach($tableObj as $oneRow){
					$parSub2LedOpt[$oneRow->ledger_id] = stripslashes(trim($oneRow->name));
				}
			}
		}
		if($post ==1){
			$jsonResponse = array();
			$jsonResponse['login'] = '';
			$jsonResponse['parSub2LedOpt'] = $parSub2LedOpt;
			return json_encode($jsonResponse);
		}
		else{
	...
	function setParSubLedOpt($parent_ledger_id, $post=0){
		...
		if($parent_ledger_id>0){
			...
			if($tableObj){
				while($oneRow = $tableObj->fetch(PDO::FETCH_OBJ)){
					$parSubLedOpt[$oneRow->ledger_id] = stripslashes(trim($oneRow->name));
				}
			}
		}
		...
		if($post ==1){
			$jsonResponse = array();
			$jsonResponse['login'] = '';
			$jsonResponse['parSubLedOpt'] = $parSubLedOpt;
			return json_encode($jsonResponse);
		}
		else{
			return $parSubLedOpt;
	...
	function AJsaveLedger(){
		...
		if($prod_cat_man==0){
			...
			$account_type = $POST['account_type']??0;
			$sub_group_id = $POST['sub_group_id']??0;
			$parent_ledger_id = $POST['parent_ledger_id']??0;
			$parent_sub_ledger_id = $POST['parent_sub_ledger_id']??0;
			$parent_sub2_ledger_id = $POST['parent_sub2_ledger_id']??0;
			$name = addslashes(trim($POST['name']??''));
			$visible_on = implode(',', $POST['visible_on']??array());
			$debit = $POST['debit']??-1;
			$credit = $POST['credit']??1;
	...
	public function AJ_ledgerview_moreInfo($ledger_id){
		...
		if(!empty($oneRow)){
			...
			$jsonResponse['name'] = stripslashes(trim($oneRow->name));
			$jsonResponse['sub_group_id'] = intval($oneRow->sub_group_id);
			$jsonResponse['debit'] = intval($oneRow->debit);
			$jsonResponse['credit'] = intval($oneRow->credit);
			
			$sub_group_id = $oneRow->sub_group_id;
			$subGroupName = $this->getOneFieldById('sub_group', $sub_group_id, 'name');
			$jsonResponse['subGroupName'] = $subGroupName;
			$parent_ledger_id = $oneRow->parent_ledger_id;
			...
			if(!empty($impSonLedIds)){
				...
				if(!empty($parLedIds)){
					...
					foreach($parLedIds as $sparent_ledger_id=>$oneParLedInfo){
						...
						$arrow = 0;
						$sonLedIdsData[] = array($oneLedInfo[0], $oneLedInfo[1], $oneLedInfo[2], $oneLedInfo[3],  $debitCredits[$oneLedInfo[4]], $debitCredits[$oneLedInfo[5]], $oneLedInfo[6], $oneLedInfo[7], $editPer, $hidePer, $arrow, $sparent_ledger_id, $oneLedInfo[8]);
						...
						if(!empty($parLedInfo)){
							...
							foreach($parSubLedIds as $sparent_sub_ledger_id=>$oneParSubLedInfo){
								...
								$sonLedIdsData[] = array($oneLedInfo[0], $oneLedInfo[1], $oneLedInfo[2], $oneLedInfo[3],  $debitCredits[$oneLedInfo[4]], $debitCredits[$oneLedInfo[5]], $oneLedInfo[6], $oneLedInfo[7], $editPer, $hidePer, $arrow, $sparent_sub_ledger_id, $oneLedInfo[8]);
								...
								if(!empty($parSubLedInfo)){
									...
									foreach($parSub2LedIds as $sparent_sub2_ledger_id=>$oneParSub2LedInfo){
										...
										$sonLedIdsData[] = array($oneLedInfo[0], $oneLedInfo[1], $oneLedInfo[2], $oneLedInfo[3],  $debitCredits[$oneLedInfo[4]], $debitCredits[$oneLedInfo[5]], $oneLedInfo[6], $oneLedInfo[7], $editPer, $hidePer, $arrow, $sparent_sub2_ledger_id, $oneLedInfo[8]);
										...
										if(!empty($parSub2LedInfo)){													
											...
											foreach($sledgerIds as $sledger_id=>$oneLedInfo){
												...
												$sonLedIdsData[] = array($oneLedInfo[0], $oneLedInfo[1], $oneLedInfo[2], $oneLedInfo[3],  $debitCredits[$oneLedInfo[4]], $debitCredits[$oneLedInfo[5]], $oneLedInfo[6], $oneLedInfo[7], $editPer, $hidePer, $arrow, $sledger_id, $oneLedInfo[8]);
	...
	private function Voucher1Data(){
		...
		if($CountList == 'Count'){
			...
			if($dataObj){
				...
				$tableRows = array($vDateNote, $voucherData, $LedIds);
			}
			return $tableRows;
		}
	}
	...
	public function voucherPrint(){
		...
		if($voucher_id==0){
			...
			$domainName = implode('.',array_slice($serverexp, -3, 3));
	...
	public function AJsaveVoucher2(){
		...
		if($prod_cat_man==0){
			...
			$pi_invoice_no = $POST['pi_invoice_no']??0;
			$pi_invoice_date = date('Y-m-d', strtotime($POST['pi_invoice_date']??'1000-01-01'));
			$lc_phone_no = $POST['lc_phone_no']??0;
			$lc_date = date('Y-m-d', strtotime($POST['lc_date']??'1000-01-01'));
	...
	public function dayBook(){
		...
		<div class=\"flexSpaBetRow\">
			...
			<div class=\"col-sm-4 col-md-2 pright0\">
				<select name=\"fsub_group_id\" id=\"fsub_group_id\" class=\"form-control changeLoadFilterData\">
					$subGroOpt
				</select>
			</div>
			<div class=\"col-sm-4 col-md-2 pright0\">
	...
	private function ledOneRowStr($oneRow, $accountType, $subGroupName, $debitCredits, $voucherTypes, $a, $col=1){
		...
		<td data-title=\"Sub-Group Name\" align=\"left\">$subGroupName</td>";
			}
