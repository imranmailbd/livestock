...
async function  AJ_view_MoreInfo(){
    ...
    function afterFetch(data){
        ...
        if(data.manage_inventory_count>0 && data.product_type !== 'Labor/Services'){
            ...
            if(data.product_publish>0 && (['Standard', 'Mobile Devices'].includes(data.product_type))){
                    ...
                    addInventoryForm.appendChild(costRow);
                        ...
                        qtyRow.appendChild(qtyColumn);
                            ...
                            const qtyField= cTag('div', {class: "columnSM7"});
                                ...
                                if(data.product_type === 'Mobile Devices'){
        ...
        if(data.inventoryObj){
            ...
            if(data.allowed && data.product_type!=='Mobile Devices' && data.costPer===1){
            ...
            if(!['Mobile Devices'].includes(data.product_type)){
        ...
        if(data.product_publish===1){							
            ...
            buttonRowDiv.appendChild(similarButton);
                ...
                if((!["Labor/Services",'Mobile Devices'].includes(data.product_type))&&(data.manage_inventory_count===1)){
...
async function AJsave_PO(event){
	...
	if(product_type==='Mobile Devices'){
		...
		let errorid = document.getElementById("errmsg_bulkimei");
		errorid.innerHTML = '';
		if(bulkimei===''){
                    ...
                    if(data.product_type !==''){
						...
						divCol12 = cTag('div',{ 'class': 'columnXS12' });
							...
							let proTypOpts = {
                                ...
                                'Mobile Devices':['tablet', Translate('Mobile Devices'), Translate('Use for cellular devices with IMEI, ESN, MEID...')],
                    ...
                    if(data.product_type ===''){
                        ...
                        divTabs.appendChild(ulTabs);
                             ...
                             let divTabs1 = cTag('div',{ 'class': 'columnXS12 flexSpaBetRow','id': 'tabs-1' });
                                ...
                                divCol7.appendChild(categoryRow);
                                    ...
                                    //Manufacturer Name
                                    const manufacturerDiv = cTag('div',{ 'class': 'displayNotAll standardField MobileDevices LaborServices' });
							...
							divCol7.appendChild(productNameRow);
								...
								const colorDiv = cTag('div',{ 'class': 'displayNotAll MobileDevices' });
							...
							divCol7.appendChild(colorDiv);
								...
								//Storage
								const storageDiv = cTag('div',{ 'class': 'displayNotAll MobileDevices' });
							...
							divCol7.appendChild(storageDiv);
								...
								//physical condition
								const physicalDiv = cTag('div',{ 'class': 'displayNotAll MobileDevices' });
						...
						divTabs1.appendChild(divCol7);
							...
							divCol5.appendChild(serialDiv);
                                ...
                                //Inventory count
								const inventoryDiv = cTag('div',{ 'class': 'displayNotAll standardField MobileDevices manage_inventory_count' });
							...
							divCol5.appendChild(addDescriptionRow);
								...
								const imeiProductDiv = cTag('div',{ 'class': 'displayNotAll MobileDevices' });
                                    const imeiProductRow = cTag('div',{ 'class': 'flex' });
                                        const imeiProductColumn = cTag('div',{ 'class': 'columnXS12 roundborder bgcoolblue', 'style': "padding-left: 20px; padding-right: 20px; padding-top: 15px; padding-bottom: 15px;" });
											let pTag = cTag('p');
											pTag.innerHTML = Translate('IMEI numbers are added from the Product Information page after you save this new mobile device.');
                                        imeiProductColumn.appendChild(pTag);
                                    imeiProductRow.appendChild(imeiProductColumn);
                                imeiProductDiv.appendChild(imeiProductRow);
							divCol5.appendChild(imeiProductDiv);
...
function checkManageInventory(){
	...
	if(manage_inventory_countid.checked===true){	
		...
		if(product_type==='Mobile Devices'){
			document.querySelectorAll(".allow_backorder").forEach(oneFieldObj=>{
				if(oneFieldObj.style.display !== 'none'){
...
function checkProductType(){
	...
	if(product_type !==''){
		...
		if(product_type==='Mobile Devices'){
			document.querySelectorAll(".MobileDevices").forEach(el => {
				if(el.style.display === 'none'){
					el.style.display = '';
				}
			});
			...
			productName = document.getElementById("lbproduct_name");
			productName.innerHTML = Translate('Model Name');
                requiredField = cTag('span',{ 'class': 'required' });
				requiredField.innerHTML = '*';
			productName.appendChild(requiredField);
            document.getElementById("manage_inventory_count").checked=true;
