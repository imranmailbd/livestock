        ...
        function afterFetch(data){
            ...
            fragment.appendChild(cTag('div',{ 'id':'errorST','class':'errormsg' }));
                ...
                form.appendChild(changeStockTakeRow);
                    ...
                    changeStockTakeRow = cTag('div',{ 'class':'flex standardField MobileDevices LaborServices', 'style': "text-align: left; align-items: center;"  });
...
function replaceSTTdText(id, fieldVal, idPre, itemType){
    let node;
	if(itemType==='Mobile Devices' && idPre==='invCnt'){
	...
	if(document.querySelectorAll(".oneId").length){
        ...
        function afterFetch(data){
            if(data.returnStr==='error'){
				...
				replaceSTTdText(stock_take_items_id, inventory_count, 'invCnt', 'Mobile Devices');
			}			
			hidePopup();
        }
		return false;
