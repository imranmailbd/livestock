...
class Dashboard{
	...
	public function AJ_lists_MoreInfo(){
		...
		$totalcount = 0;
		$psql = "SELECT p.product_id FROM inventory i, product p WHERE i.accounts_id = $accounts_id AND i.product_id = p.product_id AND p.product_publish = 1 AND p.product_type != 'Mobile Devices' AND p.manage_inventory_count>0 AND i.current_inventory < i.low_inventory_alert GROUP BY p.product_id"; 
		$productObj = $this->db->query($psql, array());
		if($productObj){
			$totalcount = $productObj->rowCount();
		}
		
		$isql = "SELECT p.product_id, i.low_inventory_alert AS low_inventory_alert FROM inventory i, product p LEFT JOIN item ON (item.product_id = p.product_id AND item.in_inventory = 1) WHERE i.accounts_id = $accounts_id AND i.product_id = p.product_id AND p.product_publish = 1 AND p.product_type = 'Mobile Devices' AND p.manage_inventory_count>0 GROUP BY p.product_id HAVING COUNT(item.item_id) <= low_inventory_alert"; 
