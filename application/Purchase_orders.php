...
class Purchase_orders{
		...
		if(!empty($po_number) && $po_number>0){
			...
			if($query){
				while($oneRow = $query->fetch(PDO::FETCH_OBJ)){
					...
					if($item_type !='' && $item_type == 'cellphones'){
						$item_number = '';
		...
		if($po_id){
			...
			if($productObj){
				...
				if($inventoryObj){
					...
					if($product_type=='Mobile Devices' && $item_id>0){
   ...
   public function addProductToPOCart(){
		...
		if($poObj){				
			...
			if($fieldvalue !=''){
				...
				$sql .= " AND product_type in ('Standard', 'Mobile Devices')";
   ...
   public function removeThisPOItem(){
	...
	public function AJautoComplete_cartProductPO($segment2name){
		...
		$extrastr .= " AND p.product_type in ('Standard', 'Mobile Devices')";
		...
		if($query){
			while($onerow = $query->fetch(PDO::FETCH_OBJ)){
				...
				if($manage_inventory_count==1 && $product_type != 'Mobile Devices'){
					$stockQty = 0;
				...
				elseif(strcmp($product_type,'Mobile Devices')==0){
					$stockQty = 0;
