...
class Common{
	...
	public function updateMobileAvgCost($accounts_id, $po_items_id, $created_on){
		...
		if(!empty($itemIds)){
			$posSql = "SELECT pos.sales_datetime, pc.pos_cart_id FROM pos, pos_cart pc, pos_cart_item pci WHERE pci.item_id IN (".implode(', ', array_keys($itemIds)).") AND pos.sales_datetime >='$created_on' AND pc.item_type = 'cellphones' AND pos.invoice_no>0 AND (pos.pos_type = 'Sale' OR (pos.pos_type IN ('Order', 'Repairs') AND pos.order_status = 2)) AND pos.pos_id = pc.pos_id AND pc.pos_cart_id = pci.pos_cart_id GROUP BY pc.pos_cart_id ORDER BY pc.pos_cart_id ASC";
			...
			if($toAccountsId >0 && $toPoNo >0 && $toProductid >0){
				...
				if($toPoItemsId>0){
					...
					if(!empty($toItemIds)){
						$sqlquery2 = "SELECT pos.sales_datetime, pc.pos_cart_id FROM pos, pos_cart pc, pos_cart_item pci WHERE pci.item_id IN (".implode(', ', array_keys($toItemIds)).") AND pos.sales_datetime >='$toCreated_on' AND pc.item_type = 'cellphones' AND pos.invoice_no>0 AND (pos.pos_type = 'Sale' OR (pos.pos_type IN ('Order', 'Repairs') AND pos.order_status = 2)) AND pos.pos_id = pc.pos_id AND pc.pos_cart_id = pci.pos_cart_id GROUP BY pc.pos_cart_id ORDER BY pc.pos_cart_id ASC";
	...
	public function itCartCellphoneAveCost($po_items_id, $created_on, $costUpdate=0, $transfer = 1, $accounts_id = 0){
		...
		$sqlPOItems = "SELECT product_id, received_qty, cost FROM po_items WHERE po_items_id = $po_items_id AND item_type = 'cellphones' ORDER BY po_items_id ASC";
	...
	public function cartCellphoneAveCost($pos_cart_id, $sales_datetime, $costUpdate=0){
		...
		$IMEIStr = '';
		$sqlPosCart = "SELECT item_id, shipping_qty, ave_cost FROM pos_cart WHERE pos_cart_id = $pos_cart_id AND item_type = 'cellphones' ORDER BY pos_cart_id ASC";
...
{{Barcode}}";
	...
	public function AJautoComplete_product(){
		...
		$bindData = array();
		if($CellPhone==0){
			$extrastr .= " AND p.product_type != 'Mobile Devices'";
		}
		if($keyword_search !=''){
			$keyword_search = addslashes(trim((string) $keyword_search));
			if ( $keyword_search == "" ) { $keyword_search = " "; }
