...
class Carts{
		...
		if($posObj){
			...
			if($query){
				...
				while($row = $query->fetch(PDO::FETCH_OBJ)){
					...
					$cellphonesInfo = $serialInfo = array();
					if(in_array($item_type, array('cellphones')) || ($item_type=='product' && $require_serial_no>0)){					
						if($item_type=='cellphones'){
							...
							if($itemquery){
								while($itemrow = $itemquery->fetch(PDO::FETCH_OBJ)){
									$cellphonesInfo[] = array('pos_cart_item_id'=>$itemrow->pos_cart_item_id, 'item_id'=>$itemrow->item_id, 'item_number'=>$itemrow->item_number, 'carrier_name'=>$itemrow->carrier_name);
								}
							}
						}
					...
					if(in_array($product_type, array('Standard', 'Mobile Devices')) && $manage_inventory_count>0){
					...
					$cartData[] = array('pos_cart_id'=>$pos_cart_id, 'item_id'=>$item_id, 'item_type'=>$item_type, 'description'=>$description, 'sku'=>$sku, 'product_type'=>$product_type, 
						...
						'cellphonesInfo'=>$cellphonesInfo, 'serialInfo'=>$serialInfo, 'sales_price'=>$sales_price, 'minimum_price'=>$minimum_price, 'allow_backorder'=>$allow_backorder, 'discount_is_percent'=>$discount_is_percent,
	...
	public function categoryProductPicker(){
		...
		$sql .= " JOIN item ON (item.accounts_id = $accounts_id AND p.product_id = item.product_id AND item.in_inventory = 1 AND item.item_publish = 1) WHERE i.accounts_id = $accounts_id$filtersql AND p.product_type = 'Mobile Devices' AND i.product_id = p.product_id AND p.product_publish = 1 GROUP BY product_id 
		...
		if(in_array($frompage, array('Purchase_orders', 'Inventory_Transfer'))){
			...
			$sql .= " AND p.product_type != 'Mobile Devices'";
		}
		if($frompage == 'POS'){
	...
	public function productPicker(){
		...
		$sql .= " JOIN item ON (p.product_id = item.product_id AND item.in_inventory = 1 AND item.item_publish = 1) WHERE i.accounts_id = $accounts_id AND p.product_type = 'Mobile Devices' AND i.product_id = p.product_id AND p.product_publish = 1 GROUP BY category_id 
		...
		if($fieldvalue !='' && ($posObj || $pos_id>0)){
			...
			if(empty($action)){
				...
				if(empty($results)){
					...
					WHERE i.accounts_id = $accounts_id AND p.product_type = 'Mobile Devices' AND p.sku = :search_sku AND i.product_id = p.product_id AND p.product_publish = 1 GROUP BY p.sku";				
					if($frompage == 'POS'){
				...
				if(empty($results)){
					...
					$bindData = array();
					$sql = "SELECT p.product_id, p.product_type FROM product p, inventory i WHERE i.accounts_id = $accounts_id AND p.product_type != 'Mobile Devices' AND p.product_publish = 1";
					if($frompage == 'POS'){
						$sql .= " AND ((p.manage_inventory_count = 0 OR p.manage_inventory_count is null) OR (p.manage_inventory_count=1 AND i.current_inventory>0) OR p.allow_backorder = 1)"; 
					}
				...
				if(empty($results) && $clickYesNo==0){
					...
					if($query){	
						foreach($query as $onerow){
							...
							$newresults[] = array('label'=>$str1, 'item_id'=>$item_id, 'product_type'=>'Mobile Devices');
						}
					}
					...
					$sql .= " WHERE i.accounts_id = $accounts_id AND p.product_type = 'Mobile Devices'";
					...
					if($query1){
						foreach($query1 as $onerow){
							...
							$newresults[] = array('label'=>$label,'item_id'=>$onerow['product_id'], 'product_type'=>'Mobile Devices');
						}
					}
					...
					$sql = "SELECT p.product_id, p.sku, manufacturer.name AS manufacture, p.product_name, p.colour_name, p.storage, p.physical_condition_name 
							FROM inventory i, product p LEFT JOIN manufacturer ON (p.manufacturer_id = manufacturer.manufacturer_id) WHERE i.accounts_id = $accounts_id AND p.product_type != 'Mobile Devices'";
					if($frompage == 'POS'){
						$sql .= " AND ((p.manage_inventory_count = 0 OR p.manage_inventory_count is null) OR (p.manage_inventory_count=1 AND i.current_inventory>0) OR p.allow_backorder = 1)"; 
					}
						...
						if($product_type=='Mobile Devices'){
							...
							if($query1){
								foreach($query1 as $onerow){
									...
									$pos_cart_id=0;
									$sql2 = "SELECT * FROM pos_cart WHERE pos_id = $pos_id AND item_type ='cellphones' AND item_id = $product_id ORDER BY pos_id ASC LIMIT 0,1";
							...
							if($query){
								foreach($query as $oneproductrow){
									...
									if($allowsale>0){
										...
										$pos_cart_id = 0;
										$sql3 = "SELECT * FROM pos_cart WHERE pos_id = $pos_id AND item_type in ('product', 'cellphones') AND item_id = $item_id ORDER BY pos_id ASC LIMIT 0,1";
	...
	public function updateCartData($frompage){
		...
		if($pos_cartObj){
			$pos_cartrow = $pos_cartObj->fetch(PDO::FETCH_OBJ);
			$pos_id = $pos_cartrow->pos_id;
			if($pos_cartrow->item_type =='cellphones'){$maxqty = $shipping_qty = $pos_cartrow->shipping_qty;}
			...
			if(empty($action)){
				...
				if($productObj){
					...
					if($inventoryObj){
						...
						if($frompage=='POS' && $pos_cartrow->item_type !='cellphones'){
							$overselling = $qty;
						}
						...
						if($frompage =='Repairs' && $order_status == 1 && $pos_cartrow->item_type !='cellphones'){
							$allowsale = 1;
						}
