		...
		if(isset($_SESSION["returnsesarray"])){
			...
			if($returnsesarray !=''){
				...
				foreach($returnsesarray as $row){
					...
					$newimei_info = array();
					if($item_type=='cellphones' && $row['max_qty'] >0){
		...
		if(isset($_SESSION["returnsesarray"])){
			...
			if($returnsesarray !=''){						
				foreach($returnsesarray as $row){
					...
					if($item_type=='cellphones' && $max_qty >0 && $fpos==$pos){
						$imei_id = $row['imei_id'];
		...
		if(isset($_SESSION["returnsesarray"])){
			...
			if($returnsesarray !=''){						
				foreach($returnsesarray as $row){
					...
					if($item_type=='cellphones' && $max_qty >0){
						$imei_id = $row['imei_id'];
						if (strpos($imei_id, ', ') !== false) {
							$imeiIds = explode(', ', $imei_id);			
						}
					...
					if(!empty($returnsesarray)){
						foreach($returnsesarray as $row){
							...
							if($pos_cart_id){
								...
								$activity_feed_id = 0;
								if($item_type=='cellphones' && $imei_id !=''){											
