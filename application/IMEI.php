	...
	public function AJgetPopup(){
		...
		if($itemObj){
			...
			$productsql = "SELECT p.product_id, p.sku, manufacturer.name AS manufacture, p.product_name, p.colour_name, p.storage, p.physical_condition_name  
							...
							WHERE p.accounts_id = $prod_cat_man AND p.product_type = 'Mobile Devices' AND (p.product_publish = 1 OR (p.product_id = $product_id AND p.product_publish = 0)) GROUP BY p.product_id 
	...
	private function filterHAndOptions(){
		...
		if($shistory_type !=''){
			...
			elseif(strcmp($shistory_type, 'Sales Invoice Returned')==0){
				$filterSql = "SELECT COUNT(pos.pos_id) AS totalrows FROM pos, pos_cart, pos_cart_item WHERE pos.accounts_id = $accounts_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'cellphones' 
			...
			$filterSql = "SELECT COUNT(activity_feed_id) AS totalrows FROM activity_feed WHERE accounts_id = $accounts_id AND uri_table_name = 'item' $likeSql 
					...
					UNION ALL 
						SELECT COUNT(pos.pos_id) AS totalrows FROM pos, pos_cart, pos_cart_item WHERE pos.accounts_id = $accounts_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'cellphones' 
							 AND pos.pos_id = pos_cart.pos_id AND pos_cart.pos_cart_id = pos_cart_item.pos_cart_id AND pos_cart_item.sale_or_refund = 0 
					UNION ALL 
						SELECT COUNT(pos.pos_id) AS totalrows FROM pos, pos_cart, pos_cart_item WHERE pos.accounts_id = $accounts_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'cellphones' 
		...
		$Sql = "SELECT activity_feed_title AS afTitle FROM activity_feed WHERE accounts_id = $accounts_id AND uri_table_name = 'item' $likeSql 
			...
			UNION ALL 
				SELECT 'Sales Invoice Returned' AS afTitle FROM pos, pos_cart, pos_cart_item WHERE pos.accounts_id = $accounts_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'cellphones' 
					AND pos.pos_id = pos_cart.pos_id AND pos_cart.pos_cart_id = pos_cart_item.pos_cart_id AND pos_cart_item.sale_or_refund = 0 
			UNION ALL 
				...
				FROM pos, pos_cart, pos_cart_item WHERE pos.accounts_id = $accounts_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'cellphones' 
					AND pos.pos_id = pos_cart.pos_id AND pos_cart.pos_cart_id = pos_cart_item.pos_cart_id AND pos_cart_item.sale_or_refund = 1 
			UNION ALL 
		...
		if($shistory_type !=''){
			...
			elseif(strcmp($shistory_type, 'Sales Invoice Returned')==0){
				$filterSql = "SELECT 'posreturn' as tablename, pos.sales_datetime AS tabledate, pos.pos_id as table_id, 
						...
						WHERE pos.accounts_id = $accounts_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'cellphones' 
							 AND pos.pos_id = pos_cart.pos_id AND pos_cart.pos_cart_id = pos_cart_item.pos_cart_id AND pos_cart_item.sale_or_refund = 0";
			}
			elseif(in_array($shistory_type, array('Order Created', 'Repair Created', 'Sales Invoice Created'))){
			...
			$filterSql .= " WHERE pos.accounts_id = $accounts_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'cellphones' AND pos.pos_id = pos_cart.pos_id AND pos_cart.pos_cart_id = pos_cart_item.pos_cart_id AND pos_cart_item.sale_or_refund = 0";
			$filterSql .= " UNION ALL";
			...
			$filterSql .= " WHERE pos.accounts_id = $accounts_id AND pos_cart_item.item_id IN ($sitem_id) AND pos_cart.item_type = 'cellphones' AND pos.pos_id = pos_cart.pos_id AND pos_cart.pos_cart_id = pos_cart_item.pos_cart_id AND pos_cart_item.sale_or_refund = 1";
			$filterSql .= " UNION ALL";
