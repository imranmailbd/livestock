...
We will reply as soon as possible.
	...
	public function AJ_CellPhones_MoreInfo(){
		$POST = json_decode(file_get_contents('php://input'), true);
		$product_id = intval($POST['product_id']??0);
		$segment3name = $POST['segment3name']??'view';
									...
									<div class=\"collapse navbar-collapse\" id=\"bs-example-navbar-collapse-1\">
										<ul class=\"flexEndRow nav navbar-nav\">
											...
											if($this->display_inventory>0){
												$curCls = '';
												if($segment2name=='CellPhones'){$curCls = ' class="current"';}
												$htmlStr .= "<li$curCls><a href=\"/CellPhones\" title=\"".$this->db->translate('Mobile Devices')."\">".$this->db->translate('Mobile Devices')."</a></li>";
											}
