...
class Stock_Take{
		...
		if($stock_take_id){
			...
			if($queryObj){
				while($onerow = $queryObj->fetch(PDO::FETCH_OBJ)){
					...
					if($onerow->product_type=='Mobile Devices'){
	...
	public function AJupdateSTIICBySKU(){
		...
		$sqlquery = "SELECT sti.*, manufacturer.name AS manufacture, p.sku FROM stock_take_items sti, product p LEFT JOIN manufacturer ON (p.manufacturer_id = manufacturer.manufacturer_id) WHERE sti.stock_take_id = :stock_take_id AND sti.current_imeis LIKE CONCAT('%', :item_number, '%') AND p.product_type ='Mobile Devices' AND sti.product_id = p.product_id ORDER BY manufacture ASC, p.product_name ASC, p.colour_name ASC, p.storage ASC, p.physical_condition_name ASC LIMIT 0, 1";
		...
		if($action == 'error'){
			$sqlquery = "SELECT sti.*, manufacturer.name AS manufacture, p.sku FROM stock_take_items sti, product p LEFT JOIN manufacturer ON (p.manufacturer_id = manufacturer.manufacturer_id) WHERE sti.stock_take_id = :stock_take_id AND p.sku = :sku AND p.product_type !='Mobile Devices' AND sti.product_id = p.product_id ORDER BY manufacture ASC, p.product_name ASC, p.colour_name ASC, p.storage ASC, p.physical_condition_name ASC LIMIT 0, 1";
	...
	function showSTCartIMEI(){
		...
		$sqlquery = "SELECT sti.*, manufacturer.name AS manufacture, p.sku FROM stock_take_items sti, product p LEFT JOIN manufacturer ON (p.manufacturer_id = manufacturer.manufacturer_id) WHERE sti.stock_take_items_id = :stock_take_items_id AND p.product_type ='Mobile Devices' AND sti.product_id = p.product_id ORDER BY manufacture ASC, p.product_name ASC, p.colour_name ASC, p.storage ASC, p.physical_condition_name ASC LIMIT 0, 1";
