...
class Products{
	...
	private function filterAndOptions(){
		...
		$strextra ="SELECT p.product_id AS product_id, p.category_id, p.manufacturer_id, i.current_inventory AS current_inventory, manufacturer.name AS manufacture$addiselect FROM inventory i, product p LEFT JOIN manufacturer ON (p.manufacturer_id = manufacturer.manufacturer_id) 
					WHERE i.accounts_id = $accounts_id AND p.product_type != 'Mobile Devices' $sqlPublish $filterSql";
		...
		$strextra .= " WHERE i.accounts_id = $accounts_id AND p.product_type = 'Mobile Devices' $sqlPublish $filterSql";
		if($sdata_type=='Available'){
			...
			if($queryObj){
				...
				if($queryInvObj){
					...
					if(strcmp($product_type,'Mobile Devices')==0){
						$itemObj2 = $this->db->query("SELECT COUNT(item_id) AS current_inventory FROM item WHERE product_id = $product_id AND accounts_id = $accounts_id AND item_publish = 1 AND in_inventory = 1", array());
						if($itemObj2){
							$productData['current_inventory'] = floatval($itemObj2->fetch(PDO::FETCH_OBJ)->current_inventory);
						}
					}
					else{
    ...
    public function AJ_showProductRow(){
		...
		$sql = "SELECT p.*, manufacturer.name AS manufacture FROM product p LEFT JOIN manufacturer ON (p.manufacturer_id = manufacturer.manufacturer_id) WHERE p.accounts_id = $prod_cat_man AND p.product_publish = 1 AND p.product_type != 'Mobile Devices' AND p.sku = :sku ORDER BY p.sku ASC LIMIT 0, 1";
