...
$totalITCount = 0;
$accIDsSql = "";
$sqlquery = "SELECT COUNT(poi.po_items_id) as totalITCount FROM accounts, po, po_items poi WHERE $accIDsSql poi.item_type = 'cellphones' AND poi.check_ave_cost < '$date' AND po.transfer>0 AND accounts.accounts_id = po.accounts_id AND po.po_id = poi.po_id ORDER BY poi.po_items_id ASC";
...
//else{
	$totalITCount = 0;
	$accIDsSql = "";
	$sqlquery = "SELECT COUNT(pc.pos_cart_id) as totalITCount FROM accounts, pos, pos_cart pc WHERE $accIDsSql pc.item_type = 'cellphones' AND pc.check_ave_cost < '$date' AND pos.invoice_no>0 AND accounts.accounts_id = pos.accounts_id AND pos.pos_id = pc.pos_id ORDER BY pc.pos_cart_id  ASC";
	$query = $db->querypagination($sqlquery, array());
	if($query){
		foreach($query as $oneRow){
			$totalITCount = $oneRow['totalITCount'];
